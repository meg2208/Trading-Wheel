...
SELECT portfolio_id, a.security, a.buy_sell, a.share_amount, a.allocation, a.adj_close, a.time
	FROM ( 
		SELECT q.security, ir.operator, ir.buy_sell, ir.share_amount, ir.allocation, 
				q.adj_close, q.time, 
		...
		FROM indicator R_ind, query_data q, indicator_reference ir, raw_data_parsing rd,
				criteria c
		WHERE q.security = ir.action_security 
			AND rd.security = ir.action_security
			AND R_ind.indicator_id = ir.R_indicator_id
			AND R_ind.MVA_10_DAY = 'F'
			AND ir.L_indicator_id = c.indicator_id
			AND c.strategy_id = {0}
			AND rd.time = q.time 
